...
`AuditLogger` I/F（`audit.go:27`：`Log(ev AuditEvent)`）。`AuditEvent` は `Time`/`Kind`/`Role`/`AccountID`/`RemoteIP`/`RoomID` の**メタデータのみ**で、**通信ペイロードは絶対に含めない**（E2E・法的防衛線）。イベント種別は `connect`/`disconnect`/`room_create`/`guest_join`（`AuditJoinRequest` の値は `"guest_join"`）。`recordAction`（`server.go:115`）が `hub.Handle` の返す `Binding` を見て `room_create`/`guest_join` を RoomID 付きで記録する。実装は `Nop`/`Slog`→将来 S3（Object Lock+KMS）へ差し替え。

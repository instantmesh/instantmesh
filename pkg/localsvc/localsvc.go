...
// 「ポート番号 → 表示ラベル / 名前解決用スラッグ」という表と、その決定的な並び順だけである。
// Ollama や MCP のアプリ層プロトコルは一切解釈しない。表が空でもメッシュは成立する（候補提示が
// 空になり、ホストがポートを直接指定する導線に落ちるだけ）ため、汎用コアの境界を越えない。
// 製品固有の知識をこの 1 パッケージへ閉じ込めることで、名前空間（pkg/meshname）と DNS 応答
// （pkg/dnsmsg）は汎用のまま保たれる。
package localsvc
...
// KnownService は既知ポートと、その表示ラベル（ホストの画面に出す候補名）・名前解決用のラベル。
type KnownService struct {
	...
	Label string
	// Slug は名前解決（要件 §4.6.3）で使う DNS ラベル。`<Slug>.<ホスト名>.mesh` の先頭に入る。
	// Label をそのまま使えない（空白・大文字・日本語を含む）ため表に明示的に持たせ、
	// 「同じサービスなら毎回同じ名前」という決定性を表側で保証する。
	Slug string
}
...
var knownServices = []KnownService{
	{Port: 11434, Label: "Ollama", Slug: "ollama"},
	{Port: 1234, Label: "LM Studio", Slug: "lmstudio"},
	{Port: 8000, Label: "vLLM", Slug: "vllm"},
	{Port: 8080, Label: "Open WebUI", Slug: "openwebui"},
	{Port: 3000, Label: "開発サーバー", Slug: "dev"},
	{Port: 80, Label: "Dify", Slug: "dify"},
}
...
// NameLabel は共有サービスに与える DNS ラベルを返す。既知ポートは表のスラッグ、表にないポートは
// `port-<番号>` とし、同じポートなら常に同じラベルになるようにする（要件 §4.6.3 の「セッションを
// またいで名前が安定する」を満たすため）。有効範囲外のポートは空文字を返す。
func NameLabel(port int) string {
...
// SharedName は共有中サービス 1 件の到達名（要件 §4.6.2 経路(1)）。
type SharedName struct {
	// Port は共有元のポート番号。
	Port int `json:"port"`
	// Label は既知ポートの表示ラベル（表にないポートは空文字）。
	Label string `json:"label,omitempty"`
	// Name は到達名（FQDN。例 "ollama.tanaka.mesh"）。
	Name string `json:"name"`
}

// Advertise は共有中ポート一覧から、ホストが配布する名前群と共有サービスの到達名を組み立てる。
//
// names の先頭はホスト自身の名前（`<hostLabel>.mesh`）で、以降が各サービスの名前。ホスト自身の
// 名前を常に含めるのは、名前解決がピア単位の仕組みであり、一度通ればそのピアの全ポート・全
// プロトコルに等しく効くため（付録C.7）。並び順は Candidates と同じ規則で決定的にする。
//
// hostLabel は呼び出し側が meshname.Sanitize 等で LDH ラベルへ落としたもの。ポートが有効範囲外、
// ラベルが不正、名前数が上限超過の場合はエラーを返す。
func Advertise(hostLabel string, ports []int) ([]string, []SharedName, error) {
	cands, err := Candidates(nil, ports)
	if err != nil {
		return nil, nil, err
	}
	hostName, err := meshname.FQDN(hostLabel)
	if err != nil {
		return nil, nil, err
	}

	names := make([]string, 0, len(cands)+1)
	names = append(names, hostName)
	shared := make([]SharedName, 0, len(cands))
	for _, c := range cands {
		// ポートは Candidates が、ホストラベルは直前の FQDN が検証済みなので連結してよい
		// （組み上がった名前は最後の ValidateNames でまとめて検証する）。
		name := NameLabel(c.Port) + "." + hostName
		names = append(names, name)
		shared = append(shared, SharedName{Port: c.Port, Label: c.Label, Name: name})
	}
	// 上限・重複・構文をまとめて検証する（配布前に不正な広告を作らない）。
	names, err = meshname.ValidateNames(names)
	if err != nil {
		return nil, nil, err
	}
	return names, shared, nil
}

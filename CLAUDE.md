...
> **2026-07-27 方向転換**: 前面を「汎用の一時メッシュVPN」から上記へ寄せた。汎用メッシュ機能（任意ポート・汎用ルーム）は削らず土台として残す。判断の根拠・却下した代替案・残存リスクは `docs/要件定義書.md` **付録C** が正。共有層（§4.6/§4.7）は**未実装**。
...
# GUI（既定モード。ブラウザで localhost の LocalAPI を開き、ホスト作成/参加・承認を操作）
# 引数なし起動は -mode gui。127.0.0.1:8088（-gui-addr）を既定ブラウザで自動的に開く
go run ./cmd/client -server ws://localhost:8080/ws -cognito-domain= -account dev-account  # -tunnel/-stun は既定有効(要管理者)

# ホスト（ヘッドレス CLI。ルーム作成 → 招待リンク/SAS 出力 → 標準入力で承認/拒否）
# ローカルの DevAuthenticator サーバーに繋ぐため -cognito-domain= で Cognito を無効化する
go run ./cmd/client -mode host  -server ws://localhost:8080/ws -cognito-domain= -account dev-account  # 権限なしでシグナリングのみは -tunnel=false
...
- **STUN 共有ソケット**: `-tunnel` 起動時は `sharedBind`（wireguard-go の `conn.Bind` をラップ）＋ `pkg/stunmux` で **WireGuard と同一の UDP ソケットから STUN を実施**する。これにより得られる WAN マッピングが WG 送信のマッピングと一致し hole punching が成立する。`-tunnel` 無効時は別ソケット STUN（`discoverWANStandalone`）へフォールバック（マッピングがずれうる）。
...
### 共有層とゲストの到達手段（要件 §4.6・**未実装**）

方向転換で追加された層。メッシュ到達性の上に「特定のローカルサービスを、相手の手元から扱える形で出す」導線を載せる。ゲストの到達経路は3つで、**(1) を主とする**。

1. **名前解決（主）** `http://ollama.tanaka.mesh:11434` … **権威はホスト、解決はローカル**。ホストが定義した名前⇄メッシュIPの写像を既存シグナリング（`pkg/signaling` の `PeerInfo`）で配布し、各クライアントが自プロセス内に持つレスポンダで即答する。OS へは split DNS で当該サフィックスのみ注入（`cmd/client/dnsconfig_<os>.go`: Windows=NRPT／macOS=`/etc/resolver/`／Linux=systemd-resolved 等）。**`hosts` 書き換えとホスト側DNSサーバー方式は採用しない**（要件 付録C.4）。DNS メッセージの生成/解析は `pkg/stun` と同じく純粋側、ソケットは `cmd` 側に置く。
...
実装時の必須事項: split DNS の**起動時残骸回収**、当該サフィックス**以外のクエリ経路を変更しない**スコープ厳守、プロキシの**即時解放**（ホストが元ポートを占有し続けるとゲストが自分の同種サービスを起動できない）、ホスト側サービスに `0.0.0.0` バインドを要求しないこと。

### GUI（クライアントの LocalAPI 層）

クライアントは既定で **GUI モード**（`-mode gui`）で起動する。`runGUI`（`cmd/client/guiserver.go`）が 127.0.0.1（既定 `:8088`・`-gui-addr`）に HTTP サーバーを立て、既定ブラウザで自動的に開く（`cmd/client/openbrowser.go`）。埋め込み SPA（`cmd/client/guiindex.go`・外部依存なしの自己完結 HTML/JS）が `GET /api/state` をポーリングして表示状態を購読し、`POST /api/{host,join,approve,reject,rotate,leave,reset}` で操作する。
...
- **Windows 実行**: クライアント exe は GUI サブシステム（リンク `-H windowsgui`）でビルドし、ダブルクリック起動でコンソールを開かない。コマンドプロンプト等から起動された場合のみ親コンソールへ入出力を接続する（`cmd/client/attachconsole_{windows,other}.go`）。加えて UAC 昇格マニフェストを COFF リソース（`rsrc`→`rsrc_windows_amd64.syso`）として CI が生成・埋め込む。
...
6. **リレー通信量監視**: リレー転送ループ内で送信量をアトミックに加算し、プラン別の累計上限（`pkg/plan` の `RelayByteLimit`・無料プランは 100MB）に到達したら `RelayThrottledBps`（64kbps）へ**速度制限**する。**切断はしない**（要件 §4.5・付録B B.3 の決定。実装も `pkg/relay.Meter` はスロットルのみで close しない）。
7. **Redis セッション管理（AWS 展開時）**: Redis にルーム情報を登録する際は要件定義の保持時間（例: 無料1時間・有料24時間）に基づく TTL（Expire）を必ず設定する。
8. **`pkg/` に製品固有・プロトコル固有の知識を入れない**: `pkg/` の汎用コアが知ってよいのは「一時的な承認制メッシュ」までで、Ollama・LM Studio・MCP といった**製品名やアプリ層プロトコルの解釈を持ち込まない**。サービス検出のプローブ・ポート写像の適用・プロトコル解釈は `cmd/client`（I/O アダプタ）側、または汎用コアと明確に分離した上位パッケージに置く。
   - 純粋ロジック（既知ポート表の判定・ポート衝突の解決・写像定義の検証など）を `pkg/` に置くこと自体は可。境界は「**その知識が無くてもメッシュとして成立するか**」。
   - 目的は、ローカルAI/MCP という賭けが外れても土台（A）を無傷で別用途へ振り直せるようにすること。設計原則1（UI とコアの分離）の延長であり、`docs/要件定義書.md` 付録C の D-2 に対応する。

## 実装スキル（作業前に参照）

領域別・横断の実装規約を `.claude/skills/`（正本）に整備している。Claude Code が自動ロードする。作業内容に対応するスキルを実装前に読むこと。

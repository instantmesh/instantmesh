...
6. **リレー通信量監視**: リレー転送ループ内で送信量をアトミックに加算し、上限（100MB）超過時は接続インスタンスを直ちにクローズしてリソース解放漏れを防ぐ（`pkg/relay`）。
7. **Redis セッション管理（AWS 展開時）**: Redis にルーム情報を登録する際は要件定義の保持時間（例: 1時間・3時間）に基づく TTL（Expire）を必ず設定する。

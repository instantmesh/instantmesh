...
go run ./cmd/client -mode host  -server ws://localhost:8080/ws -account dev-account [-tunnel -stun stun.l.google.com:19302]

# ゲスト（招待リンクから参加 → 帯域外MITM照合）
go run ./cmd/client -mode guest -invite "instantmesh://join?..." -nick alice [-tunnel -stun ...]
```

`-tunnel` は wireguard-go 仮想NICを起動する（要管理者/root 権限）。`-relay` は P2P 直通失敗時のリレー自動フォールバック（既定 true・要 `-tunnel`）。
